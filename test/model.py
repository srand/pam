...
class ModelTest(unittest.TestCase):
    ...
    def test_cxxproject_source(self):
        for toolchain in ToolchainRegistry.this_system():
            ...
            lib.add_sources('src/test_cxx_source.cpp')
            lib.add_sources('src', regex=r'.*source_regex\.cpp$')
            lib.add_sources('src', regex='.*never_matched.*')
            lib.add_sources('src/test_cxx_source_tool.unknown', tool='.cpp')
            lib.transform(toolchain)

            exe = CXXExecutable('test_cxxexe_source-{}'.format(toolchain.name))
            exe.add_sources('src/test_cxx_source_link.cpp')
    ...
    def test_cxxproject_macro(self):
        for toolchain in ToolchainRegistry.this_system():
            ...
            lib.add_sources('src/test_cxx_macros.cpp')
            lib.transform(toolchain)

            exe = CXXExecutable('test_cxxexe_macro-{}'.format(toolchain.name))
            exe.add_sources('src/test_cxx_macros_link.cpp')
            exe.add_dependency(lib)
            exe.transform(toolchain)
    ...
    def test_cxxproject_incpath(self):
        for toolchain in ToolchainRegistry.this_system():
            ...
            lib.add_sources('src/test_cxx_incpath.cpp')
            lib.add_incpath('inc_private')
            lib.add_incpath('inc_public', publish=True)
            lib.add_incpath('inc_bad', filter='never_matched')
            lib.transform(toolchain)

            exe = CXXExecutable('test_cxxexe_incpath-{}'.format(toolchain.name))
            exe.add_sources('src/test_cxx_incpath_link.cpp')
            exe.add_dependency(lib)
            exe.transform(toolchain)
